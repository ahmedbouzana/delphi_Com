...
type
  TForm_approvisionnement = class(TForm)
    ...
    DBEdit_num_d_sr: TDBEdit;
    DBEdit_num_f_sr: TDBEdit;
...
uses Unit_datamodule, DateUtils;

{$R *.dfm}

procedure TForm_approvisionnement.FormActivate(Sender: TObject);
...
begin
  ...
  'from approvisionner, type_cpt  where approvisionner.code_type = type_cpt.code_type and approvisionner.num_dem_app ='+ DBEdit_numero.Text;
...
code_type:string;
begin
  if (DBLookupComboBox_nom_type.KeyValue=null) or (DBEdit_qte_app.Text='') or (DBEdit_num_d_sr.Text='') or (DBEdit_num_f_sr.Text='') then
    ...
    numero := DBEdit_numero.Text;
     ...
     if DataModule1.ADOQuery_approvisionnement.State in [dsInsert] then
      ...
      //DataModule1.ADOQuery_approvisionnement.Post;
        DataModule1.ADOQuery_approvisionnement.SQL.Text := 'update approvisionner set qte_app = ' + DBEdit_qte_app.Text + ', num_d_sr = ''' + DBEdit_num_d_sr.Text + ''' , num_f_sr = ''' + DBEdit_num_f_sr.Text + ''' where id = '+DBEdit_id_app.Text;
      DataModule1.ADOQuery_approvisionnement.ExecSQL;
